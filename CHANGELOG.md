...
- 存档 Schema 由 8 提升至 9、内容版本提升至 `2026.08.1`；Schema 8 的 `probationEndsAtDay` 确定性迁移为显式 `CurrentAppointment.probation` 生命周期。
...
- 可迁移范围扩展为 Schema 2–9；Schema 8 活动试用期迁移后保留原截止日，既有非试用任职迁移为 `probation: null`。迁移失败继续保留原始只读备份。
