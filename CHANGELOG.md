...
- 行动实例稳定 ID 与启动时职位/机构/地区快照；行动完成后发出 `action.completed`。
...
- 存档 Schema 由 5 提升至 6。
- Schema 5→6 将 `time.pendingContinuation` 初始化为 null，并为各层级执行中行动生成 `legacy-action-{tier}-{slotIndex}-{startedAtDay}-{actionId}`，使用旧存档当前任职补齐冻结来源。
