...
- NPC 自主补员：非初始编制 Vacancy 在玩家机会缺位达到 `npcStaffingDelayDays`（30 天）后，由 NPC-only 相对选拔自主产生唯一赢家、完成任职并级联原岗位空缺，组织世界不因玩家不行动而停摆；失败 attempt 保留审计并按 `npcStaffingRetryIntervalDays`（30 天）退避重试，不永久封死空缺。
...
- 新录用公务员试用期完整生命周期：配置化期限、条件、评价阈值、单次延期上限与不予转正后果；统一绝对日时间轴在到期日精确评估，并持久化评估审计。
- 职业页展示试用状态、剩余天数、转正条件进度、延期/失败原因和转正反馈；工作台同步显示阶段提醒。
- Playwright 试用期纵向验收，覆盖建档、完成最低行动要求、跨到期日转正与刷新恢复。
- 科员个人任务、年度职数、两次独立公务员职级晋升，以及乡科级副职/正职机会与 270 天选拔窗口。
- 副职基层治理内容，含招商项目、产业园政策/危机和防汛准备正式事件链。
- Phase 3 producer/consumer 固定点审计、任务运行时前置边界、阶段预算与正向 KPI producer 门禁。
- 不改写业务存档的四年 Playwright 自然路径，以及 390×844 移动端核心操作验收。
- 全新「政务 · 案牍」设计系统：红头文件式总栏、宋体标题、宣纸/墨夜双主题（亮暗切换持久化）。
- 全局导航条：工作台 / 任务 / 部门治理 / 年度考核 / 职务职级 / 政策 / 事件，事件与岗位机会数量角标。
- 干部档案卡、公文式页面标题、统一卡片/标签/进度条/弹窗组件体系。
- 建档向导重做为「干部履历表」版面。

### Changed

- 软件版本提升为 `0.3.0-alpha.1`；存档 Schema 最终为 10，内容版本提升至 `2026.08.7`。
- Schema 8→9 将 `probationEndsAtDay` 迁移为显式试用生命周期；Schema 9→10 初始化个人任务冷却、完成计数和总数。
- 全部正式页面与启动页按新设计系统重写，移除内联配色。
...
- 可迁移范围扩展为 Schema 2–10；Schema 8 活动试用期保留原截止日，Schema 9 个人任务状态确定性初始化。
- Schema 10 内容版本按 `.2→.3→.4→.5→.6→.7` 链式升级；`.6→.7` 不改写既有预算、机会、事件、政策或在途快照。迁移失败继续保留原始只读备份。
