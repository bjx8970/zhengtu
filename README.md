...
当前是**本地单机优先的可玩原型**，已存在行政线 L1-L11 的配置与 L11 终局代码（36 个职位），但连续晋升链路尚未通过真实验证（当前存在 `multi_region` 履历判定阻塞）。玩家可以完成六步建档，在 Dashboard 查看日程并进入部门治理页安排行动、推进时间、完成 KPI 与年度考核，并通过晋升任命页选择目标职位后完成晋升流程。
...
| 职业发展 | 行政线 L1-L11 配置已存在（36 个职位），但连续晋升链路未验证；晋升前从候选职位中显式选择目标 |
