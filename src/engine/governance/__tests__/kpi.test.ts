...
describe('calculateKPI', () => {
  describe('ratio 型指标（上限 1.5）', () => {
    it('恰好达到目标：完成率 1.0', () => {
      ...
      const result = calculateKPI(indicators, depts);
...
describe('scoreToKPITier', () => {
  it('>= 90 → 优秀', () => {
    expect(scoreToKPITier(90)).toBe(KPITier.Excellent);
    expect(scoreToKPITier(95)).toBe(KPITier.Excellent);
  });

  it('>= 75 → 称职', () => {
    expect(scoreToKPITier(75)).toBe(KPITier.Competent);
    expect(scoreToKPITier(89)).toBe(KPITier.Competent);
  });

  it('>= 60 → 基本称职', () => {
    expect(scoreToKPITier(60)).toBe(KPITier.Basic);
    expect(scoreToKPITier(74)).toBe(KPITier.Basic);
  });

  it('< 60 → 不称职', () => {
    expect(scoreToKPITier(59)).toBe(KPITier.Incompetent);
    expect(scoreToKPITier(0)).toBe(KPITier.Incompetent);
  });
});
