...
describe('computeEffect', () => {
  describe('normal path', () => {
    ...
    it('multiply 操作翻倍数值', () => {
      const effect: ActionEffectDef = { target: 'player.performance', operation: 'multiply', value: 2 };
  ...
  describe('edge cases', () => {
    it('未知 operation 返回原值', () => {
      const effect = { target: 'player.stability', operation: 'unknown' as ActionEffectDef['operation'], value: 5 };
...
describe('applyEffectMap', () => {
  ...
  describe('edge cases', () => {
    ...
    it('target 在 currentValues 中不存在时从 0 开始', () => {
      const effects: ActionEffectDef[] = [
        { target: 'x', operation: 'add', value: 3 },
      ];
