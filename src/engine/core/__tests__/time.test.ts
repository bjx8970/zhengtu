...
describe('advanceTime', () => {
  describe('basic advancement', () => {
    it('推进 1 天：日期 +1', () => {
      const result = advanceTime(makeState(), 1, 1990, 3, cfg);
    ...
    it('推进 0 天：状态不变化', () => {
      ...
      const result = advanceTime(original, 0, 1990, 3, cfg);
  ...
  describe('month boundary', () => {
    it('月末推进触发月度结算', () => {
      const result = advanceTime(makeState({ day: 30 }), 1, 1990, 3, cfg);
    ...
    it('29 → 31 → 下月', () => {
      const result = advanceTime(makeState({ day: 29 }), 2, 1990, 3, cfg);
      expect(result.newState.day).toBe(1);
      expect(result.newState.month).toBe(7);
    });

    it('月中推进不触发月度结算', () => {
      const result = advanceTime(makeState({ day: 15 }), 5, 1990, 3, cfg);
  ...
  describe('year boundary', () => {
    it('年末推进触发年度考核', () => {
      const result = advanceTime(makeState({ month: 12, day: 30 }), 1, 1990, 3, cfg);
    ...
    it('跨多年推进：多个年度触发', () => {
      const result = advanceTime(makeState({ month: 12, day: 1 }), 400, 1990, 3, cfg);
  ...
  describe('5-year cycles', () => {
    it('2025 年（5 的倍数）触发两会', () => {
      const result = advanceTime(makeState({ year: 2024, month: 12, day: 30 }), 1, 1990, 3, cfg);
      expect(result.triggers.some((t) => t.type === 'congress_cycle')).toBe(true);
    });

    it('非 5 的倍数年不触发两会', () => {
      const result = advanceTime(makeState({ year: 2023, month: 12, day: 30 }), 1, 1990, 3, cfg);
  ...
  describe('retirement check', () => {
    it('到 65 岁触发退休', () => {
      ...
      const result = advanceTime(state, 1, birthYear, 3, cfg);
      expect(result.triggers.some((t) => t.type === 'retirement_check')).toBe(true);
    });

    it('未到 65 岁不触发退休', () => {
      const birthYear = 2000;
      const result = advanceTime(makeState(), 365, birthYear, 3, cfg);
  ...
  describe('sentiment generation', () => {
    it('rank4+ 在月边界触发舆情生成', () => {
      const result = advanceTime(makeState({ day: 30 }), 1, 1990, 7, cfg);
  ...
  describe('edge cases', () => {
    it('负天数：抛出异常', () => {
      expect(() => advanceTime(makeState(), -1, 1990, 3, cfg)).toThrow();
    });

    it('极大天数：允许快进多年', () => {
      const result = advanceTime(makeState(), 3650, 1990, 3, cfg);
...
describe('utility functions', () => {
  ...
  it('getRetirementCountdown computes correctly', () => {
    expect(getRetirementCountdown(2024, 1990, cfg.retirementAge)).toBe(31);
    expect(getRetirementCountdown(2024, 1959, cfg.retirementAge)).toBe(0);
  });

  it('isCongressYear detects correctly', () => {
    expect(isCongressYear(2025, cfg.congressCycleYears)).toBe(true);
    expect(isCongressYear(2024, cfg.congressCycleYears)).toBe(false);
    expect(isCongressYear(2030, cfg.congressCycleYears)).toBe(true);
  });
});
