 ...
 * reducer 内部全部走 `payload._rng ?? Math.random` / `payload._idFactory ?? createRuntimeIdFactory(...)`
 * 的回退模式，生产端 dispatch 通常不携带这两个字段，因此必须**总是注入**
 * 包装实现（调用方已提供时包装之，未提供时包装默认实现），
 * 否则回退分支会产生未录制的随机数/ID，破坏确定性重放。
 *
 * 本模块不修改任何业务参数，仅包裹/补充函数字段。
 */
 ...
 *
 * 对支持这两个字段的动作总是注入：字段缺失或非函数时以默认实现
 * （Math.random / runtime ID 工厂）为源，保证 reducer 不再走未记录的回退分支。
 *
 * @param action 原始动作
